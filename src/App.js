...
const translations = {
  en: { title: "Fundamental Rights", subtitle: "UPSC CSE • Polity • Part III", tagline: "Articles 12–35", visualTab: "Visual Journey", cheatTab: "Cheat Sheet", mastered: "Mastered All 6 Rights!", footer: "Made with ❤️ for UPSC Warriors • Jai Hind!",
    rights: [
      ...
      { title: "Constitutional Remedies", articles: "Article 32", mnemonic: "Heart & Soul - Dr. Ambedkar", details: ["Art 32: Supreme Court Access", "5 Writs: Habeas, Mandamus, Prohibition, Certirai, Quo-Warranto"] }
    ]
  },
  hi: { title: "मौलिक अधिकार", subtitle: "UPSC CSE • राज्यशास्त्र • भाग-III", tagline: "अनुच्छेद १२-३५", visualTab: "दृश्य यात्रा", cheatTab: "चीट शीट", mastered: "सभी 6 अधिकार पूर्ण!", footer: "UPSC योद्धाओं के लिए ❤️ • भारत माता की जय!",
    rights: [
  ...
  mr: { title: "मूलभूत अधिकार", subtitle: "UPSC CSE • राज्यशास्त्र • भाग-३", tagline: "कलम १२-३५", visualTab: "दृश्य प्रवास", cheatTab: "चीट शीट", mastered: "सर्व ६ अधिकार पूर्ण!", footer: "UPSC योद्ध्यांसाठी ❤️ • जय हिंद! जय महाराष्ट्र!",
    rights: [
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white font-sans p-4">
      ...
      <div className="fixed right-6 top-20 z-50 flex flex-col gap-3">
        <motion.button whileHover={{ scale: 1.1 }} onClick={cycleLang} className="p-3 bg-gray-800 rounded-full shadow-2xl border border-gray-700">
          <Globe className="w-6 h-6" />
          <span className="text-xs block">{lang.toUpperCase()}</span>
        </motion.button>
        <motion.button whileHover={{ scale: 1.1 }} onClick={() => window.print()} className="p-3 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-full shadow-2xl print:hidden">
      ...
      {completedCards.size === 6 && (
        <motion.div initial={{ y: -50 }} animate={{ y: 0 }} className="fixed top-20 left-1/2 -translate-x-1/2 bg-gradient-to-r from-amber-500 to-pink-500 px-6 py-3 rounded-full shadow-2xl flex items-center gap-2 z-50">
      ...
      <header className="pt-24 pb-8 text-center">
        <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }}>
          <span className="inline-block bg-gradient-to-r from-orange-500 to-red-600 px-5 py-1.5 rounded-full text-sm font-bold mb-4">
            {t.subtitle}
          </span>
          <h1 className="text-5xl md:text-7xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400 mb-3">
            {t.title}
          </h1>
          <p className="text-xl text-gray-400">{t.tagline}</p | t.tagline}</p>
        </motion.div>
      </header>
      ...
      <div className="flex justify-center gap-4 mb-10">
        {['grid', 'cheatsheet'].map(tab => (
          <motion.button key={tab} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setActiveTab(tab)}
            className={`px-8 py-3 rounded-xl font-bold text-lg ${activeTab === tab ? 'bg-gradient-to-r from-indigo-600 to-purple-600' : 'bg-gray-800 hover:bg-gray-700'} transition-all`}>
            {tab === 'grid' ? t.visualTab : t.cheatTab}
          </motion.button>
        ))}
      </div>
          ...
          {t.rights.map((right, i) => (
            <motion.div key={i+1} initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }}
              ...
              <div className={`relative rounded-3xl overflow-hidden shadow-2xl border border-gray-800 h-full bg-gradient-to-br ${cardColors[i]} p-1`}>
                <div className="bg-gray-900/95 backdrop-blur-sm rounded-3xl p-8 h-full flex flex-col">
                  <div className="flex justify-between items-start mb-6">
                    <div className="p-4 bg-white/10 rounded-2xl backdrop-blur">{icons[i]}</div>
                    {completedCards.has(i+1) && <CheckCircle2 className="w-8 h-8 text-green-400" />}
                  </div>
                  <h3 className="text-2xl font-bold mb-3 text-white">{right.title}</h3>
                  <p className="text-gray-300 mb-4 text-lg">{right.articles}</p>
                  <p className="bg-white/10 px-4 py-2 rounded-full text-sm font-medium mb-6 inline-block">{right.mnemonic}</p>
              ...
              <AnimatePresence>
                {expandedId === i+1 && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                    className="mt-4 bg-gray-900/90 backdrop-blur border border-gray-800 rounded-2xl overflow-hidden">
                    <div className="p-6">
      ...
      <footer className="text-center py-12 text-gray-500 text-sm mt-20">
        {t.footer}
      </footer>
    </div>
  );
