...
const translations = {
  en: {
    ...
    footer: "Made with love for UPSC Warriors - Jai Hind!"
  },
  hi: {
    title: "मौलिक अधिकार",
    ...
    cheatTab: "चीट शीट",
    rights: [
      { title: "समता का अधिकार", articles: "अनुच्छेद 14-18", mnemonic: "L-D-O-U-T", details: ["अनु.14: विधि के समक्ष समता", "अनु.15: भेदभाव निषेध", "अनु.16: सरकारी नौकरी में समान अवसर", "अनु.17: अस्पृश्यता उन्मूलन", "अनु.18: उपाधियाँ समाप्त"] },
      ...
      { title: "धर्म की स्वतंत्रता", articles: "अनुच्छेद 25-28", mnemonic: "अंतःकरण-प्रचार-व्यवस्था-कर-बाध्यता", details: ["अनु.25: अंतःकरण व प्रचार", "अनु.26: धार्मिक कार्य प्रबंधन", "अनु.27: धर्म हेतु कर नहीं", "अनु.28: धार्मिक शिक्षा बाध्यता नहीं"] },
    ...
    footer: "UPSC योद्धाओं के लिए प्यार से - भारत माता की जय!"
  },
  mr: {
    title: "मूलभूत अधिकार",
    ...
    footer: "UPSC योद्ध्यांसाठी प्रेमाने - जय हिंद! जय महाराष्ट्र!"
  }
};
...
const FundamentalRights = () => {
  ...
  const colors = ["from-blue-500 to-cyan-500", "from-green-500 to-emerald-600", "from-red-500 to-pink-600", 
                  "from-purple-500 to-indigo-600", "from-amber-500 to-orange-600", "from-slate-700 to-slate-900"];
  ...
  return (
    <div className={`min-h-screen transition-colors duration-700 ${darkMode ? 'bg-gray-950 text-gray-100' : 'bg-gradient-to-br from-indigo-50 via-white to-purple-50'} font-sans p-4`}>
      ...
      <div className="fixed right-6 top-20 z-50 flex flex-col gap-3">
        <motion.button whileHover={{ scale: 1.1 }} onClick={cycleLang} className="p-3 bg-white dark:bg-gray-800 rounded-full shadow-2xl">
          <Globe className="w-6 h-6" />
          <span className="text-xs block">{lang.toUpperCase()}</span>
        </motion.button>
        <motion.button whileHover={{ scale: 1.1 }} onClick={() => setDarkMode(!darkMode)} className="p-3 bg-white dark:bg-gray-800 rounded-full shadow-2xl">
      ...
      <div className="flex justify-center gap-4 mb-8">
        {['grid', 'cheatsheet'].map(tab => (
          <motion.button key={tab} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setActiveTab(tab)}
            className={`px-6 py-3 rounded-xl font-bold ${activeTab === tab ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white' : 'bg-white dark:bg-gray-800'}`}>
      ...
      {activeTab === 'grid' && (
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
          {t.rights.map((right, i) => (
            <motion.div key={i+1} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              whileHover={{ y: -8 }} onClick={() => toggleExpand(i+1)} className="cursor-pointer">
              <div className={`relative rounded-2xl overflow-hidden shadow-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} border border-gray-200 dark:border-gray-700`}>
                <div className={`absolute inset-0 bg-gradient-to-br ${colors[i]} opacity-90`} />
                <div className="relative p-6 text-white">
                  <div className="flex justify-between mb-4">
                    <div className="p-3 bg-white/20 rounded-xl">{icons[i]}</div>
                    {completedCards.has(i+1) && <CheckCircle2 className="w-7 h-7 text-green-400" />}
                  </div>
                  ...
                  <motion.div animate={{ rotate: expandedId === i+1 ? 180 : 0 }} className="flex justify-end">
